...
class LstmCnnGate(nn.Module):
    def __init__(self,
        ...
        # word representation level
        self.word_gate = nn.Linear(self.word_dim, self.word_dim)
        self.char_gate = nn.Linear(self.word_dim, self.word_dim)

        # feature extraction level
        # context-only feature linear layers
        self.cof_linear_fwd = nn.Linear(self.uni_lstm_size, self.uni_lstm_size)
        self.cof_linear_bwd = nn.Linear(self.uni_lstm_size, self.uni_lstm_size)
        # hidden states gates
        self.hs_gates = nn.ModuleList([
            nn.Linear(self.uni_lstm_size, self.uni_lstm_size),
            nn.Linear(self.uni_lstm_size, self.uni_lstm_size)])
